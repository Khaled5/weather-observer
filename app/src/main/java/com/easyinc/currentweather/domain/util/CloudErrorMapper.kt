package com.easyinc.currentweather.domain.util

import com.easyinc.currentweather.common.Logger
import com.easyinc.currentweather.domain.util.response.ErrorModel
import com.easyinc.currentweather.domain.util.response.ErrorStatus
...
class CloudErrorMapper @Inject constructor(private val gson: Gson) {
    ...
    fun mapToDomainErrorException(throwable: Throwable?): ErrorModel {
        val errorModel: ErrorModel? = when (throwable) {
            ...
            else -> ErrorModel(throwable?.message,4,ErrorStatus.NOT_DEFINED)
